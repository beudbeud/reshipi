...
class CubeLut(val size: Int = 33) {
     ...
     *
     * [notes] become comment lines under the header. A cube is only meaningful
     * against the encoding and the rendering it was measured in, and the format
     * has nowhere to declare either: an application picks an input space by
     * guessing, and Darktable's guess for a .cube is Rec.709 where this one is
     * sRGB. Saying it in the file is the only place it can be said.
     */
    fun toCubeText(
        title: String,
        notes: List<String> = emptyList(),
        grid: FloatArray = build(),
    ): String = buildString {
